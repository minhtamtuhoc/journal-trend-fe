...
export function KpiCard({ label, value, delta, hint, accent = false }: { label: string; value: string; delta?: string; hint?: string; accent?: boolean }) {
  return (
    <div className="glass rounded-2xl p-6 relative overflow-hidden group hover:border-brand/30 transition-colors">
      ...
      <p className={`text-3xl font-bold text-foreground font-mono ${accent ? "bg-clip-text text-transparent" : ""}`} style={accent ? { backgroundImage: "var(--gradient-brand)" } : undefined}>
        {value}
      </p>
